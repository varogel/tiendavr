...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (JSON.status) {
        ...
        document.getElementById('imagen').src = SERVER_URL.concat('images/productos/', JSON.dataset.foto);
        document.getElementById('nombre').textContent = JSON.dataset.titulo;
        ...
        document.getElementById('id_producto').value = JSON.dataset.id;
    } else {
...
SHOPPING_FORM.addEventListener('submit', async (event) => {
    ...
    if (JSON.status) {
        sweetAlert(1, JSON.message, false, 'carrito.html');
