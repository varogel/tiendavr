        ...
        switch ($_GET['action']) {
            ...
            case 'update':
                ...
                if (!$producto->setId($_POST['id'])) {
                    ...
                    $result['exception'] = Validator::getFileError();
                } elseif ($producto->updateRow($data['foto'])) {
                    $result['status'] = 1;
                    if (Validator::saveFile($_FILES['archivo'], $producto->getRuta(), $producto->getImagen())) {
                        $result['message'] = 'Producto modificado correctamente';
                    } else {
            ...
            case 'delete':
                if (!$producto->setId($_POST['id'])) {
                    ...
                    $result['status'] = 1;
                    if (Validator::deleteFile($producto->getRuta(), $data['foto'])) {
                ...
                break;
            case 'cantidadProductosCompras':
                if ($result['dataset'] = $producto->cantidadProductosCompras()) {
                    $result['status'] = 1;
                } else {
